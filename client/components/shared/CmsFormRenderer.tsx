      ...
      <div>
        <div className="inline-block border-2 border-brand-accent p-1 w-full hover:border-black transition-all duration-300 hover:bg-white">
          <Button
            ...
            className="w-full bg-brand-accent border-2 border-black px-6 py-3 font-poppins text-[18px] font-normal uppercase text-black hover:bg-white hover:text-black hover:border-white transition-all duration-300 rounded-none h-auto flex items-center justify-center gap-2 whitespace-nowrap"
          >
...
function FormField({ field }: { field: FormFieldDef }) {
  switch (field.type) {
    ...
    case "checkbox":
      return (
        <fieldset>
          <legend className="font-poppins text-[16px] text-[#6b6b6b] mb-2">
            {field.label}
          </legend>
          {(field.options ?? []).map((opt) => (
            <label
              key={opt}
              className="flex items-center gap-2 font-poppins text-[15px] text-[#6b6b6b] mb-1 cursor-pointer"
            >
              <input
                type="checkbox"
                name={field.name}
    ...
    case "file":
      return (
        <div>
          <label className="block font-poppins text-[16px] text-[#6b6b6b] mb-1">
            {field.label}
          </label>
          <input
            type="file"
            ...
            required={field.required}
            accept={field.accept}
            className="w-full text-[#6b6b6b] text-[16px] file:mr-4 file:py-2 file:px-4 file:border file:border-[#c4c4c4] file:bg-[#f7f7f7] file:text-[#6b6b6b] file:text-sm file:font-poppins file:rounded-none"
          />
        </div>
      );
    ...
    case "html":
      return (
        <div
          className="font-poppins text-[15px] text-[#6b6b6b] [&_a]:text-blue-600 [&_a]:underline"
